...
async function getResults(query) {
  await fetch(
    `${api.base}/forecast?q=${query}&appid=${api.key}&cnt=7&units=metric`
  ) // forecast
    .then((response) => {
...
async function displayResults(response) {
  ...
  temp.innerHTML = `<span class="current-temp">${Math.round(
  ...
  wind.innerHTML = `Wind: ${Math.round(
    Number(response.list[0].wind.speed) * kmh
  ...
  ytemps.push(
    ...
    Math.round(response.list[4].main.temp),
    Math.round(response.list[5].main.temp),
    Math.round(response.list[6].main.temp)
  );
  addClass();
  massPopChart.update();
}
...
async function getGeoResults(coordinates) {
  ...
  await fetch(
    `${api.base}/forecast?lat=${latitude}&lon=${longitude}&appid=${api.key}&cnt=7&units=metric`
  )
    .then((response) => {
      return response.json();
    })
...
async function getNews() {
  await fetch(
    "https://weather338.p.rapidapi.com/news/list?offset=0&limit=10",
    options
  )
    .then((response) => {
